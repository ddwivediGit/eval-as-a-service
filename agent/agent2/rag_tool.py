"""
rag_tool.py — Agent 2: Financial & SaaS Subscription Agent
ChromaDB vector retrieval scoped to financial_policies collection.
"""
import os
import math
...
# Agent 2 defaults: financial_policies collection + financial_documents dir
DEFAULT_DOCS_DIR = Path(__file__).resolve().parent.parent.parent / "data" / "financial_documents"
CHROMA_DIR = Path(__file__).resolve().parent.parent.parent / "data" / "chroma_db"
DEFAULT_COLLECTION = "financial_policies"
...
def load_all_documents(docs_dir: Optional[Path] = None) -> List[Dict[str, str]]:
    """Loads all markdown document files from the financial documents directory."""
...
def init_chroma_collection(collection_name: str = DEFAULT_COLLECTION, docs_dir: Optional[Path] = None):
    """Initializes persistent ChromaDB collection and indexes financial policy document chunks."""
...
def search_documents(query: str, top_k: int = 3, collection_name: str = DEFAULT_COLLECTION, docs_dir: Optional[Path] = None) -> List[Dict[str, Any]]:
    """Searches the financial_policies ChromaDB collection for relevant context chunks."""
            ...
            if results and results["documents"] and len(results["documents"][0]) > 0:
                ...
                for d, m, chunk_id, dist in zip(docs, metas, ids, distances):
                    ...
                    retrieved_chunks.append({
                        ...
                        "doc_title": m.get("doc_title", "Financial Policy Document"),
                        "chunk_id": chunk_id,
                        "text": d,
                        "score": sim_score,
