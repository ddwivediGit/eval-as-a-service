"""
graph.py — Agent 2: Financial & SaaS Subscription Agent
LangGraph workflow hardcoded to the 'financial_saas' agent domain.
"""
import time
import os
import re
...
from agent.agent2.state import AgentState
from agent.agent2.sql_tool import get_db_schema, execute_sql_query, generate_llm_sql, validate_sql_schema_alignment
from agent.agent2.rag_tool import search_documents
from agent.registry import get_agent_config

load_dotenv()

# This agent is exclusively for the Financial & SaaS domain
AGENT_ID = "financial_saas"
...
def classify_llm_intent(question: str, api_key: Optional[str] = None, provider: str = "Groq") -> Tuple[str, str]:
    """Classifies query intent for the Financial SaaS agent using LLM or heuristic fallback."""
    ...
    if effective_key:
        prompt = f"""You are an AI Agent Router for the 'financial_saas' domain.
Classify the following user question into one of three categories:
- 'sql': structured database queries about metrics, revenue, subscriptions, accounts, invoices, usage, API calls, plans, prices.
- 'rag': questions about SaaS policies, uptime SLA, refund terms, SOC2 compliance, security, encryption, privacy, support.
...
{{"intent": "sql" | "rag" | "direct", "reasoning": "short explanation"}}"""
    ...
    # Fallback heuristic for Financial SaaS
    q = question.lower()
    sql_keywords = ["revenue", "subscription", "invoice", "usage", "account", "api call", "tier", "plan", "highest", "lowest", "overdue", "saas"]
    rag_keywords = ["policy", "sla", "uptime", "security", "privacy", "terms", "soc2", "encryption", "limit", "refund", "support"]
...
def router_node(state: AgentState, api_key: Optional[str] = None, provider: str = "Groq") -> AgentState:
    """LangGraph Node 1: Router Node — classifies intent for Financial SaaS agent."""
...
def sql_execution_node(state: AgentState, api_key: Optional[str] = None, provider: str = "Groq") -> AgentState:
    """LangGraph Node 2A: Text-to-SQL Node — queries the financial_saas database."""
    start_time = time.time()
    question = state["question"]
...
def rag_retrieval_node(state: AgentState) -> AgentState:
    """LangGraph Node 2B: Document Vector RAG Retrieval Node — searches financial_policies."""
    start_time = time.time()
    question = state["question"]
...
def synthesizer_node(state: AgentState, api_key: Optional[str] = None, provider: str = "Groq") -> AgentState:
    ...
    if effective_key:
        try:
            # Every route reaches the same final-answer LLM, including direct
            # questions and tool calls that found no data.
            if intent == "sql":
                ctx_summary = (
                ...
                ctx_summary = (
                    "You are the Financial & SaaS Subscription Agent. You can answer general questions "
                    "about your capabilities, and can use SQL for subscription data or RAG for company policies."
                )

            prompt = f"""You are the final response generator for a Financial & SaaS Subscription Agent.
    ...
    if intent == "sql":
        ...
        if sql_err:
            ...
            if len(sql_res) == 1 and len(sql_res[0]) == 1:
                ...
                if isinstance(val, (float, int)) and ("revenue" in col.lower() or "fee" in col.lower() or "amount" in col.lower()):
                    answer = f"Based on the financial database records, the total value for **{col}** is **${val:,.2f}**."
                else:
                ...
                answer = f"Here are the query results from our financial database:\n\n" + "\n".join(summary_lines)

    elif intent == "rag":
        context = state.get("retrieved_context", [])
        if not context:
            answer = "I searched our ChromaDB vector store, but couldn't find specific matching financial policy guidelines."
        else:
            top_chunk = context[0]
            vdb = top_chunk.get("vector_db", "ChromaDB")
            answer = f"Based on our official financial policy document (**{top_chunk['doc_title']}** retrieved via {vdb}):\n\n"
            answer += top_chunk["text"].replace("# ", "### ")
            answer += f"\n\n*(Vector Source: {top_chunk['source']} | Similarity Score: {top_chunk['score']})*"

    else:
        answer = f"Hello! I am the Financial & SaaS Subscription Agent. I use ChromaDB vector retrieval for company policies and Text-to-SQL for database queries about subscriptions, invoices, and usage metrics."
...
def run_agent_workflow(question: str, api_key: Optional[str] = None, provider: str = "Groq", agent_id: str = AGENT_ID) -> AgentState:
    """Executes the Financial SaaS Agent LangGraph workflow with ChromaDB and state tracing."""
