"""
sql_tool.py — Agent 2: Financial & SaaS Subscription Agent
Text-to-SQL tooling scoped to the financial_saas SQLite database.
"""
import sqlite3
import re
import os
...
# Agent 2 default DB: financial_saas
DEFAULT_DB_PATH = Path(__file__).resolve().parent.parent.parent / "data" / "financial_saas.db"
AGENT_ID = "financial_saas"


def get_db_schema(db_path: Optional[Path] = None) -> str:
    """Returns formatted schema of all tables in the financial_saas SQLite database."""
...
def execute_sql_query(sql: str, db_path: Optional[Path] = None) -> Tuple[Optional[List[Dict[str, Any]]], Optional[str]]:
    """Executes a SQL query against the financial_saas database and returns (rows, error)."""
    target_path = db_path or DEFAULT_DB_PATH
    if not target_path.exists():
...
def validate_sql_schema_alignment(sql: str, agent_id: str = AGENT_ID) -> Tuple[bool, List[str]]:
    """Validates if tables and columns in generated SQL exist in the financial_saas schema."""
    from agent.registry import get_agent_config
    config = get_agent_config(agent_id)
    known_tables = config.get("known_tables", ["subscriptions", "invoices", "usage_metrics", "plans"])
...
def generate_llm_sql(question: str, api_key: Optional[str] = None, provider: str = "Groq", db_path: Optional[Path] = None) -> Tuple[str, str]:
    """Generates Text-to-SQL query for the Financial SaaS domain using Groq, Gemini, or OpenAI."""
    ...
    if effective_key:
        system_prompt = f"""You are an expert Text-to-SQL AI Agent for the Financial & SaaS Subscription SQLite database.
Database Schema:
{schema}
...
def generate_mock_sql(question: str, schema: str = "") -> Tuple[str, str]:
    """Generates SQL deterministically for Financial SaaS domain queries."""
    q = question.lower()
    ...
    if "subscription" in q or "saas" in q or "account" in q or "plan" in q or "invoice" in q:
        ...
        sql = "SELECT * FROM subscriptions LIMIT 5;"
        reasoning = "Default Financial SaaS subscriptions table query."
