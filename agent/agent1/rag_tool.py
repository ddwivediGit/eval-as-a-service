"""
rag_tool.py — Agent 1: E-Commerce Operations Agent
ChromaDB vector retrieval scoped to ecommerce_policies collection.
"""
import os
import math
...
# Agent 1 defaults: ecommerce_policies collection + documents dir
DEFAULT_DOCS_DIR = Path(__file__).resolve().parent.parent.parent / "data" / "documents"
CHROMA_DIR = Path(__file__).resolve().parent.parent.parent / "data" / "chroma_db"
DEFAULT_COLLECTION = "ecommerce_policies"
...
def load_all_documents(docs_dir: Optional[Path] = None) -> List[Dict[str, str]]:
    """Loads all markdown document files from the ecommerce documents directory."""
...
def init_chroma_collection(collection_name: str = DEFAULT_COLLECTION, docs_dir: Optional[Path] = None):
    """Initializes persistent ChromaDB collection and indexes ecommerce policy document chunks."""
...
def search_documents(query: str, top_k: int = 3, collection_name: str = DEFAULT_COLLECTION, docs_dir: Optional[Path] = None) -> List[Dict[str, Any]]:
    """Searches the ecommerce_policies ChromaDB collection for relevant context chunks."""
