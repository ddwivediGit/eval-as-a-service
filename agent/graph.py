...
from agent.sql_tool import get_db_schema, execute_sql_query, generate_llm_sql, validate_sql_schema_alignment
...
def classify_llm_intent(question: str, api_key: Optional[str] = None, provider: str = "Groq", agent_id: str = "ecommerce") -> Tuple[str, str]:
    """Classifies query using Groq LLM (or OpenAI/Gemini) if API key available, or heuristic rules."""
    ...
    if effective_key:
        prompt = f"""You are an AI Agent Router for the '{agent_id}' domain.
Classify the following user question into one of three categories:
- 'sql': structured database queries about metrics, revenue, tables, numbers, customers, accounts, orders, subscriptions, prices, usage.
- 'rag': questions about store/company policies, rules, refund window, warranty terms, SLA guarantees, shipping fees, privacy, support, SOC2 compliance.
...
{{"intent": "sql" | "rag" | "direct", "reasoning": "short explanation"}}"""
    ...
    # Fallback heuristic
    q = question.lower()
    sql_keywords = ["revenue", "sales", "price", "stock", "highest", "lowest", "spent", "orders", "customer", "delivered", "shipped", "tier", "subscription", "invoice", "usage", "account"]
    rag_keywords = ["policy", "refund", "return", "warranty", "claim", "shipping fee", "delivery time", "security", "privacy", "terms", "sla", "uptime", "soc2", "limit"]
...
def router_node(state: AgentState, api_key: Optional[str] = None, provider: str = "Groq", agent_id: str = "ecommerce") -> AgentState:
    """LangGraph Node 1: Router Node."""
    start_time = time.time()
    question = state["question"]
    
    intent, reasoning = classify_llm_intent(question, api_key=api_key, provider=provider, agent_id=agent_id)
...
def sql_execution_node(state: AgentState, api_key: Optional[str] = None, provider: str = "Groq", agent_id: str = "ecommerce") -> AgentState:
    """LangGraph Node 2A: Text-to-SQL Node."""
    start_time = time.time()
    question = state["question"]
...
def rag_retrieval_node(state: AgentState, agent_id: str = "ecommerce") -> AgentState:
    """LangGraph Node 2B: Document Vector RAG Retrieval Node (ChromaDB)."""
    start_time = time.time()
    question = state["question"]
...
def synthesizer_node(state: AgentState, api_key: Optional[str] = None, provider: str = "Groq") -> AgentState:
    """LangGraph Node 3: Synthesizer Node."""
    start_time = time.time()
    intent = state["intent"]
    question = state["question"]
    ...
    if effective_key:
        try:
            if intent == "sql":
                ...
                ctx_summary = "The agent can answer general capability questions, use SQL for database facts, and use RAG for policy documents."

            prompt = f"""You are the final response generator for a domain assistant.
Answer the user's question directly and clearly. Use only supplied context for factual claims.
If a tool result is empty or failed, explain that plainly. Do not invent facts.
...
Question: {question}"""
    ...
    if intent == "sql":
        ...
        answer = "Hello! I can answer general questions, use ChromaDB vector retrieval for company policies, and use Text-to-SQL for database queries."
...
def run_agent_workflow(question: str, api_key: Optional[str] = None, provider: str = "Groq", agent_id: str = "ecommerce") -> AgentState:
    """Executes the LLM Q&A Agent workflow with ChromaDB and state tracing."""
    ...
    s1 = router_node(initial_state, api_key=api_key, provider=provider, agent_id=agent_id)
    if s1["intent"] == "sql":
        s2 = sql_execution_node(s1, api_key=api_key, provider=provider, agent_id=agent_id)
    elif s1["intent"] == "rag":
        s2 = rag_retrieval_node(s1, agent_id=agent_id)
    else:
        s2 = s1
